...
class ResBlock(nn.Module):
    ...
    def __init__(self, channels=None, dropout=0.1, double_in=False, time_emb=512):
        """
        :param channels:
        :param dropout:
        ...
        # -- If we have double the input channels, then the residual connection within the block needs to project down.
        #    We do this with a simple pointwise conv.
...
class VCResBlock(ResBlock):
    """
    Res block for var. cold diffusion. Adds a second output projected up to double the channels
    """

    def __init__(self, *args, **kwargs):
        """
        """
        super().__init__(double_in=True, *args, **kwargs)
        # -- VC res blocks always have double the input channel, because they add the output of the corresponding block
        #    in the encoder branch to their input.

        self.varout = nn.Conv2d(self.channels, self.channels * 2, kernel_size=1, padding=0)
    def forward(self, x, *args, **kwargs):

        res = super().forward(x, *args, **kwargs)

        return res, self.varout(res)
...
class UNet(nn.Module):
    def __init__(self,
            ...
            disc_time=False         # If true, time is discrete (positive ints) else, continuous in (0, 1)
        ):
        super().__init__()
        ...
        if disc_time:
            # Discrete time, use embedding vectors
            ...
            self.timeembs = nn.Sequential(
                nn.Linear(1, time_emb * 2), nn.ReLU(),
    ...
    def forward(self, x, time):
        ...
        time = self.timeembs(time) # This is either an embedding layer or a feedforward network
...
class VCUNet(nn.Module):
    ...
    def __init__(self,
            ...
            num_blocks = 3,         # Number of res blocks per level
            mid_layers = 3,         # Number of linear layers in the middle block
            res_cat=False,          # whether to concatenate the res connections (if False, they are summed)
            time_emb=512,           # Time embedding dimension
        ):
        super().__init__()
        ...
        for i, ch in enumerate(channels):
            ...
            self.encoder.extend(ResBlock(channels=ch, time_emb=time_emb)     for _ in range(num_blocks))
            self.vcencoder.extend(VCResBlock(channels=ch, time_emb=time_emb) for _ in range(num_blocks))
        ...
        for i, ch in enumerate(rchannels):
            ...
            # Add a sequence of ResBlocks
            self.decoder.extend(ResBlock(channels=ch, double_in=res_cat, time_emb=time_emb) for _ in range(num_blocks))
    ...
    def forward(self, x1, t0, t1, x0=None):
        ...
        for mod in self.decoder:
            if type(mod) == ResBlock:
                ...
                if x0 is None:
                    z = torch.randn_like(h) # sample from the standard Gaussian
                else:
                ...
                x = mod(torch.cat([x, h, z], dim=1), time) if self.res_cat else mod(x + h + z, time)
            else:
                x = mod(x)
